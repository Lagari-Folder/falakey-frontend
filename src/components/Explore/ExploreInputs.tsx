...
const ExploreInputs = () => {
  ...
  const { data } = useFetchFilter(searchParams.toString()); // Fetch based on URL params
    ...
    <div className="flex flex-col gap-3 ">
      ...
      {openFilter && (
        <div>
          ...
          {/* <Select
          value={selectedSort}
          className="max-h-[50px] w-[50%] !rounded-xl"
          onChange={(e) => {
            dispatch(search({ ...previousSearchParams, sort: e.target.value }));
            handleSortChange(e.target.value);
          }}
        >
          {data?..map((orient) => (
            <MenuItem value={orient.key}>{orient.label}</MenuItem>
          ))}
        </Select> */}
          {/* <div></div> */}
        </div>
